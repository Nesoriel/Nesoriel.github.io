...
The domestic site and the global site should share the same main pages, same project portfolio, same Notes, same Pulse content, and same organization identity.
...
- Position Notes as Records / Decisions, not as an automatic activity feed.

### 5.3 Nesoriel Pulse

Pulse is automatic lab activity data, not handwritten blog content and not a separate content mode.

Suggested locations:

```txt
src/data/pulse/
scripts/collect-pulse.mjs
scripts/pulse-repositories.mjs
src/config/pulse.ts
```

Rules:

- Pulse data source files are `src/data/pulse/*.json`.
- Pulse data is generated by `scripts/collect-pulse.mjs`.
- Repository tracking should be maintained in `scripts/pulse-repositories.mjs`, and in `src/config/pulse.ts` if the implementation needs site-side config.
- Generated Pulse JSON must be stable, reviewable, and safe to commit.
- Do not include secrets, tokens, private repository details, or other sensitive information in Pulse output.
- If there is no recent activity, generate a clear no-activity summary instead of fabricated content.
- Pulse must follow the same dual-domain same-content rules as Projects and Notes.
...
## 10.1 Pulse collection workflow
...
- Support scheduled runs with `schedule`.
- Support manual runs with `workflow_dispatch`.
- Use `GITHUB_TOKEN` for GitHub API / repository access.
- Set workflow permissions to include:
...
- Commit only generated, non-sensitive Pulse data.
...
- How to add a note
- How Pulse works and how to add tracked repositories
...
- Pulse data generation and documentation are accurate.
- Search works.
- RSS works.
- Sitemap works.
